...
#define SO2_WE_SAMPLE_BUFFER      (0)
#define SO2_AUX_SAMPLE_BUFFER     (1)
#define A_PM1P0_SAMPLE_BUFFER     (2)
#define A_PM2P5_SAMPLE_BUFFER     (3)
#define A_PM10P0_SAMPLE_BUFFER    (4)
#define B_PM1P0_SAMPLE_BUFFER     (5)
#define B_PM2P5_SAMPLE_BUFFER     (6)
#define B_PM10P0_SAMPLE_BUFFER    (7)
#define NUM_SAMPLE_BUFFERS        (8)
float so2_ppb = 0.0f;
float instant_so2_we_v = 0.0f;
float instant_so2_aux_v = 0.0f;
boolean so2_we_ready = false;
boolean so2_aux_ready = false;
boolean init_so2_we_adc_ok = false;
boolean init_so2_aux_adc_ok = false;
void set_so2_slope(char * arg);
void set_so2_offset(char * arg);
void set_so2_sensitivity(char * arg);
void so2_we_baseline_voltage_characterization_command(char * arg);
void so2_aux_baseline_voltage_characterization_command(char * arg);
void so2_negz(char * arg);
const char cmd_string_so2_sen[] PROGMEM     = "so2_sen    ";
const char cmd_string_so2_slope[] PROGMEM   = "so2_slope  ";
const char cmd_string_so2_off[] PROGMEM     = "so2_off    ";
const char cmd_string_so2_we_blv[] PROGMEM  = "so2we_blv  ";
const char cmd_string_so2_aux_blv[] PROGMEM = "so2aux_blv ";
const char cmd_string_so2_negz[] PROGMEM    = "so2_negz   ";
...
#define EEPROM_CO_SENSITIVITY     (EEPROM_SO2_CAL_OFFSET - 4)     // float value, 4-bytes, the sensitivity from the sticker
...
#define EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE (EEPROM_NTP_TZ_OFFSET_HRS - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_MQTT_TOPIC_SUFFIX_ENABLED  (EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE - 1)    // a simple flag to indicate whether or not the topic suffix is enabled
...
#define EEPROM_SO2_ZERO_NEGATIVE_RESULTS (EEPROM_DISABLE_SOFTAP - 1)
#define EEPROM_CO_ZERO_NEGATIVE_RESULTS (EEPROM_SO2_ZERO_NEGATIVE_RESULTS - 1)
...
#define EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE (EEPROM_PM10P0_CAL_OFFSET - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_TEMPERATURE_OFFLINE_OFFSET (EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE - 4)
...
#define EEPROM_BACKUP_PM10P0_CAL_OFFSET  (EEPROM_BACKUP_SO2_CAL_OFFSET + 4)
...
PGM_P const commands[] PROGMEM = {
    ...
    cmd_string_so2_sen,
    cmd_string_so2_slope,
    cmd_string_so2_off,
    cmd_string_so2_we_blv,
    cmd_string_so2_aux_blv,
    cmd_string_so2_negz,
...
void (*command_functions[])(char * arg) = {
    ...
    set_so2_slope,
    set_so2_offset,
    so2_we_baseline_voltage_characterization_command,
    so2_aux_baseline_voltage_characterization_command,
    so2_negz,
    set_pm1p0_offset,
    set_pm2p5_offset,
    set_pm10p0_offset,
...
void updateDisplayedSensors() {
    ...
    if(first || ((current_millis - previous_display_update) >= display_update_interval)) {
        ...
        default:
            ...
            if(init_so2_we_adc_ok && init_so2_aux_adc_ok) {
                if(so2_we_ready && so2_aux_ready || (sample_buffer_idx > 0)) {
                    updateLCD(so2_ppb, 5, 1, 3, false);
                }
                else {
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |     Alpha SO2 / PM Sensor Suite    |"));
    ...
    selectSlot1();
    Serial.print(F("Info: SO2 Sensor WE ADC Initialization..."));
    if(MCP342x::errorNone == adc.convert(MCP342x::channel1, MCP342x::oneShot, MCP342x::resolution16, MCP342x::gain1)) {
        Serial.println(F("OK."));
        init_so2_we_adc_ok = true;
    }
    else {
        Serial.println(F("Failed."));
        init_so2_we_adc_ok = false;
    }

    selectSlot2();
    Serial.print(F("Info: SO2 Sensor Aux ADC Initialization..."));
    if(MCP342x::errorNone == adc.convert(MCP342x::channel1, MCP342x::oneShot, MCP342x::resolution16, MCP342x::gain1)) {
        Serial.println(F("OK."));
        init_so2_aux_adc_ok = true;
    }
    else {
        Serial.println(F("Failed."));
        init_so2_aux_adc_ok = false;
    }
...
// sampling settings
    ...
    if((l_sampling_interval == 0xFFFF) || (l_reporting_interval == 0xFFFF) || (l_averaging_interval == 0xFFFF)) {
        ...
        configInject(F("sampling 5, 480, 60\r"));
    }
...
void print_eeprom_value(char * arg) {
    if (strncmp(arg, "mac", 3) == 0) {
        ...
        print_label_with_star_if_not_backed_up("SO2 Sensitivity [mV/ppb]: ", BACKUP_STATUS_SO2_CALIBRATION_BIT);
        print_eeprom_float((const float *) EEPROM_SO2_SENSITIVITY);
        ...
        print_eeprom_float((const float *) EEPROM_SO2_CAL_OFFSET);
        Serial.print(F("    "));
        Serial.print(F("SO2 Report Negative values as zero: "));
        ...
        Serial.print(F("    "));
        Serial.println(F("SO2 Baseline Voltage WE Characterization:"));
        print_baseline_voltage_characterization(EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE);
        Serial.print(F("    "));
        Serial.println(F("SO2 Baseline Voltage Aux Characterization:"));
        print_baseline_voltage_characterization(EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE);
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        configInject(F("sampling 5, 480, 60\r")); // sample every 5 seconds, average over 10 minutes, report every minute
        ...
        eeprom_write_block("aqev2_alpha_so2_pm_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean add_baseline_voltage_characterization(char * arg, uint32_t eeprom_table_base_address) {
    ...
    if(!empty_location_found) {
        ...
        if(eeprom_table_base_address == EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE) {
            Serial.print(F("so2we"));
        }
        else {
            Serial.print(F("so2aux"));
        }
...
void so2_we_baseline_voltage_characterization_command(char * arg) {
    baseline_voltage_characterization_command(arg, EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE);
}

void so2_aux_baseline_voltage_characterization_command(char * arg) {
    baseline_voltage_characterization_command(arg, EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE);
}
...
void so2_negz(char * arg) {
    ...
    if(strcmp(arg, "0") == 0) {
        eeprom_write_byte((uint8_t *) EEPROM_SO2_ZERO_NEGATIVE_RESULTS, 0);
    }
    else {
        eeprom_write_byte((uint8_t *) EEPROM_SO2_ZERO_NEGATIVE_RESULTS, 1);
    }
    recomputeAndStoreConfigChecksum();
}
...
void collectSO2(void) {
    if(init_so2_we_adc_ok && init_so2_aux_adc_ok) {
        selectSlot1();
        if(burstSampleADC(&instant_so2_we_v)) {
            addSample(SO2_WE_SAMPLE_BUFFER, instant_so2_we_v);
        ...
        selectSlot2();
        if(burstSampleADC(&instant_so2_aux_v)) {
            addSample(SO2_AUX_SAMPLE_BUFFER, instant_so2_aux_v);
            if(sample_buffer_idx == (sample_buffer_depth - 1)) {
                so2_aux_ready = true;
            }
        }
...
void so2_convert_from_volts_to_ppb(float we_volts, float aux_volts, float * converted_value, float * temperature_compensated_value) {
    static boolean first_access = true;
    // static float so2_zero_volts = 0.0f;
    static float so2_slope_ppb_per_volt = 0.0f;
    static boolean so2_report_neg_as_zero = true;
    ...
    if(first_access) {
        ...
        so2_report_neg_as_zero = eeprom_read_byte((const uint8_t *) EEPROM_SO2_ZERO_NEGATIVE_RESULTS) ? true : false;
        first_access = false;
    }
    ...
    if(valid_temperature_characterization(EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE)) {
        ...
        if(find_and_load_temperature_characterization_entry(EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE, temperature_degc)) {
            ...
            load_temperature_characterization_entry(EEPROM_SO2_WE_BASELINE_VOLTAGE_TABLE, 0);
            we_baseline_offset_voltage_at_temperature =
    ...
    if(valid_temperature_characterization(EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE)) {
        // do the math a little differently in this case
        // first figure out what baseline_offset_voltage_at_temperature is based on the characterization
        // (1) find the first entry in the table where temperature_degc is >= the table temperature
        // (2) if no such entry exists, then it is colder than the coldest characterized temperature
        // (3) use the associated slope and intercept to determine baseline_offset_voltage_at_temperature
        //     using the formula baseline_offset_voltage_at_temperature = slope * temperature_degc + intercept
        if(find_and_load_temperature_characterization_entry(EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE, temperature_degc)) {
            // great we have a useful entry in the table for this temperature, pull the slope and intercept
            // and use them to evaluate the baseline voltage at the current temperature
            ...
            // in this special case, instead of extrapolating, we will just evaluate the entry line
            // at coldest characterized temperature, and use that
            load_temperature_characterization_entry(EEPROM_SO2_AUX_BASELINE_VOLTAGE_TABLE, 0);
            aux_baseline_offset_voltage_at_temperature =
                baseline_voltage_struct.temperature_degC * baseline_voltage_struct.slope_volts_per_degC + baseline_voltage_struct.intercept_volts;
        }
    }
    ...
    *converted_value = we_delta_aux * so2_slope_ppb_per_volt;
    if(so2_report_neg_as_zero && (*converted_value <= 0.0f)) {
        *converted_value = 0.0f;
    }

    // ppm, corrected for offset drift, and atmospheric pressure (altitude)
    *temperature_compensated_value = we_delta_aux * so2_slope_ppb_per_volt
                                     / signal_scaling_factor_at_altitude;
    //                                   / signal_scaling_factor_at_temperature       // not characterized

    if(so2_report_neg_as_zero && (*temperature_compensated_value <= 0.0f)) {
...
boolean publishSO2() {
    clearTempBuffers();
    uint16_t num_samples = so2_we_ready && so2_aux_ready ? sample_buffer_depth : sample_buffer_idx;
    float converted_value = 0.0f, compensated_value = 0.0f;
    float so2_we_moving_average = calculateAverage(&(sample_buffer[SO2_WE_SAMPLE_BUFFER][0]), num_samples);
    float so2_aux_moving_average = calculateAverage(&(sample_buffer[SO2_AUX_SAMPLE_BUFFER][0]), num_samples);
    so2_convert_from_volts_to_ppb(so2_we_moving_average, so2_aux_moving_average, &converted_value, &compensated_value);
    so2_ppb = compensated_value;
    safe_dtostrf(so2_we_moving_average, -8, 6, raw_value_string, 16);
    safe_dtostrf(so2_aux_moving_average, -8, 6, raw_value_string2, 16);
    safe_dtostrf(converted_value, -4, 2, converted_value_string, 16);
    safe_dtostrf(compensated_value, -4, 2, compensated_value_string, 16);
    safe_dtostrf(instant_so2_we_v, -8, 5, raw_instant_value_string, 16);
    safe_dtostrf(instant_so2_aux_v, -8, 5, raw_instant_value_string2, 16);
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"so2\",\"particulate\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
...
void printCsvDataLine() {
    ...
    Serial.print(so2_we_moving_average, 6);
    appendToString(so2_we_moving_average, 6, dataString, &dataStringRemaining);

    Serial.print(F(","));
    appendToString("," , dataString, &dataStringRemaining);

    Serial.print(so2_aux_moving_average, 6);
    appendToString(so2_aux_moving_average, 6, dataString, &dataStringRemaining);
